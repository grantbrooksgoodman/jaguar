...
public class GoogleTranslator: NSObject, Translatorable {
    ...
    public func translate(_ text: String,
                          from: String,
                          to: String,
                          using: TranslationPlatform = .google,
    ...
    private func evaluateJavaScript() {
        ...
        webView.evaluateJavaScript("document.getElementsByClassName('J0lOec')[0].innerText;") { (data, error) in
            if let error = error {
...
extension GoogleTranslator: WKNavigationDelegate {
    public func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        if webView.url!.absoluteString.hasPrefix("https://consent.google.com/") {
            //Click to agree to cookie settings.
            webView.evaluateJavaScript("document.getElementsByClassName('VfPpkd-RLmnJb')[3].click();")
        } else {
