...
class Build {
    ...
    //        guard !prefersConsistentBuildInfo || buildType == .generalRelease else {
    //            extraneousInformationView = main.extraneousInformationView
    //            sendFeedbackButton = main.sendFeedbackButton
    //
    //            extraneousInformationView.alpha = 0
    //            sendFeedbackButton.alpha = 0
    //
    //            return
    //        }
    //
    ...
    //                if subtitleExpiryString == "Evaluation period ended." && timebombActive {
    ...
    //                    main.preReleaseInformationView.alpha = (buildType != .generalRelease ? 1 : 0)
    //                    main.sendFeedbackButton.alpha = 1
    //                })
    //            } else {
    //                #warning("TODO: Update for use of constraints.")
    ...
    /**
     ...
     - Parameter controller: The **MFMailComposeViewController** with which we are dealing.
     - Parameter result: The result of the attempted sending.
     - Parameter error: An optional instance of **Error**, if one did occur.
     */
    func handleMailComposition(controller: MFMailComposeViewController,
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(1), execute: {
            ...
            } else if result == .sent {  //If the message did send.
    ...
    //        }
    //
    //        viewController.subtitleButton.isUserInteractionEnabled = timebombActive
    //    }
    ...
    private func generateProjectIdentifier() -> String {
        ...
        for individualIntegerAsString in multipliedConstantArray {
            ...
            let cipheredMiddleLetter = middleLetter.ciphered(by: Int(individualIntegerAsString)!).uppercased()
        ...
        if projectIdentifierAsStringCharacterArray.count > 8 {
            ...
            while projectIdentifierAsStringCharacterArray.count < 8 {
                ...
                letterInUse = letterInUse.ciphered(by: letterInUse.alphabeticalPosition)
